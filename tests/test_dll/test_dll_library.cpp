...
void dll_function_level1() {
    TRC_SCOPE();
    TRC_MSG("DLL Level 1 function called");
    ...
    TRC_MSG("DLL Level 1 completed work, sum = %d", sum);
}
...
void dll_function_level2() {
    TRC_SCOPE();
    TRC_MSG("DLL Level 2 function called");
    
    // Call level 1 function
    dll_function_level1();
    
    TRC_MSG("DLL Level 2 completed");
}
...
int dll_math_add(int a, int b) {
    TRC_SCOPE();
    TRC_MSG("DLL Math: Adding %d + %d", a, b);
    
    int result = a + b;
    TRC_MSG("DLL Math: Result = %d", result);
...
int dll_math_multiply(int a, int b) {
    TRC_SCOPE();
    TRC_MSG("DLL Math: Multiplying %d * %d", a, b);
...
void dll_nested_calls() {
    TRC_SCOPE();
    TRC_MSG("DLL Nested calls starting");
    ...
    TRC_MSG("DLL Nested calls completed, results: %d, %d", result1, result2);
}
